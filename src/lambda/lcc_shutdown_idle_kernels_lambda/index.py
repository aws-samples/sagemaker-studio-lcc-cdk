...
def on_create(domain_id: str, kernel_shutdown_lifecycle_config: str):
    """Function to execute when creating a new custom resource

    Args:
        domain_id (str): SageMaker Studio Domain ID
        kernel_shutdown_lifecycle_config (str): Name of the lcc
    ...
    try:
        response = sm_client.create_studio_lifecycle_config(
            StudioLifecycleConfigName=kernel_shutdown_lifecycle_config,
...
def on_update(
    domain_id: str, kernel_shutdown_lifecycle_config: str, physical_resource_id: str
):
    ...
    on_delete(kernel_shutdown_lifecycle_config, physical_resource_id)

    return on_create(domain_id, kernel_shutdown_lifecycle_config)
...
def on_delete(kernel_shutdown_lifecycle_config: str, physical_resource_id: str):
...
def is_delete_complete(kernel_shutdown_lifecycle_config: str):
    ...
    try:
        ...
        logger.exception("studio lifecylce config does not exist anymore")
        return {"IsComplete": True}
...
def on_event_handler(event, context):
    ...
    kernel_shutdown_lifecycle_config = event["ResourceProperties"][
        "kernel_shutdown_lifecycle_config"
    ]
    ...
    if request_type == "Update":
        return on_update(
            domain_id, kernel_shutdown_lifecycle_config, physical_resource_id
        )
    if request_type == "Delete":
        return on_delete(kernel_shutdown_lifecycle_config, physical_resource_id)
    raise Exception(f"Invalid request type: {request_type}")


def is_complete_handler(event, context):
    logger.info(event)
    kernel_shutdown_lifecycle_config = event.get("ResourceProperties", {}).get(
        "kernel_shutdown_lifecycle_config"
    )
    request_type = event["RequestType"]
    ...
    if request_type == "Delete":
        return is_delete_complete(kernel_shutdown_lifecycle_config)
    raise Exception(f"Invalid request type: {request_type}")
